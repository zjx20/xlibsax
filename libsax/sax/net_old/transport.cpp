...
tcp_connection* transport::connect(const char *ip, const int &port, const bool &is_tcp/* = 1*/) {
    if(is_tcp) {
        ...
        socket::set_non_block(tcp.get_fd());
        socket::set_keepalive(tcp.get_fd(), 5, 5, 3);
...
int transport::send(void* a_task, const char *ip, const int &port) {
    ...
    return _streamer->on_data_write(conn, a_task, conn->get_write_buffer());
}
