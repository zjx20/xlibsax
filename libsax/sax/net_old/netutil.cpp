...
bool epoll_mgr::add(const int &fd,const bool &read,const bool &write,const bool &error, void *data, g_eda_func *proc) {
    if(NULL == _mgr || NULL == proc) return false;
    int mask = 0;
    if(read) mask |= EDA_READ;
    if(write) mask |= EDA_WRITE;
    if(error) mask |= EDA_ERROR;
    return g_eda_add(_mgr, fd, mask, proc, data) == 0;
}

void epoll_mgr::del(const int &fd,const bool &read,const bool &write,const bool &error) {
    if(NULL == _mgr) return;
    int mask = 0;
    if(read) mask |= EDA_READ;
    if(write) mask |= EDA_WRITE;
...
void epoll_mgr::set(const int &fd,const bool &read,const bool &write,const bool &error) {
    if(NULL == _mgr) return;
    int mask = 0;
    if(read) mask |= EDA_READ;
    if(write) mask |= EDA_WRITE;
...
void epoll_mgr::call_back(g_eda_t *mgr, int fd, void *data, int mask) {
    ...
    if(mask & EDA_ERROR) {
        ctx->handle_error_event();
    }
    else if(mask & EDA_READ) {
