...
#pragma pack(pop)
	...
	struct pos {
		inline pos() : block(NULL), curr_buf(NULL), position(0), remaining(0), invalid(true)
		{}

		inline pos(buffer_block* block_, uint8_t *curr_buf_, size_t position_,
				size_t remaining_ = 0, bool invalid_ = false) :
			block(block_), curr_buf(curr_buf_), position(position_),
...
public:
	...
	inline bool preappend(size_t length) {
		...
		if(length != 0) {
			do {
				size_t tmp = length < _current.remaining ? length : _current.remaining;
				length -= tmp;
				_current.curr_buf += tmp;
				_current.remaining -= tmp;
				_current.position += tmp;
