...
#include <sax/c++/lru_btree.h>

/// lru type for use in the unit tests
typedef sax::LRU_btree<std::string,std::string> unit_lru_type;
/// lru POD type for use in the unit tests
typedef sax::LRU_btree<int,int> unit_lru_type2;
...
typedef sax::LRU_btree<int,test_big_data> unit_lru_type3;
...
bool lru_cache_1cycle() 
{
	...
	// Test LRU_btree removal as we add more members than max_size()
	L->insert( "baz", "monkey" );
	L->insert( "quz", "xyzzy" );
	unit_assert( "verify LRU_btree semantics", unit_data_1cycle_f.compare( dump( L ) ) == 0 );
