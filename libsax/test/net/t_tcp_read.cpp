...
//		g_thread_sleep(5.0);
		char buf[1024];
		//while (1) {
		int ret = g_tcp_read(fd, buf, sizeof(buf));
		total_read += ret;
		printf("first. total_read: %d ret: %d errno: %d %s\n", total_read, ret, errno, strerror(errno));
...
int main()
{
	...
	printf("set non block. ret = %d errno: %d %s\n", g_set_non_block(fd), errno, strerror(errno));
	printf("accepted.\n");
	int ret = g_tcp_read(fd, buf, sizeof(buf));
